...
@section("content")
    <div id="app">
        <section class="section">
            {!! import("admin.components.breadcrumb",['content'=>'Kayan Resim Güncelle']) !!}
            <div class="section-body">
                <form action="{{route("modules.scrollingPictures.update",$scrollingPicture->id)}}" method="post">
                @csrf
                <!-- Main Content -->
                    <div class="row">
                        <div class="col-12">
                                ...
                                <div class="card-body">
                                  ...
                                  <div class="form-group">
                                    ...
                                    <div class="form-group row mb-4">
                                        <input type="text" value="{{$scrollingPicture->id}}" name="id" hidden>
                                        <input type="text" value="{{$scrollingPicture->img_url}}" name="title" hidden>
                                        <div class="form-group col-6 ml-auto mr-auto mb-4">
                                            <div class="section-title mt-0">Durum</div>
                                            <select class="form-control" name="status">
                                                <option value="1" {{$scrollingPicture->status==1 ? "selected" : null }} >Aktif</option>
                                                <option value="0" {{$scrollingPicture->status==0 ? "selected" : null }} >Pasif</option>
                                            </select>
                                        </div>
                                    </div>
...
@section("script")
    ...
    <script>
        ...
        document.getElementById('image_img').src ="{{$scrollingPicture->img_url}}";
        document.getElementById('image_label').value = "{{$scrollingPicture->img_url}}";
