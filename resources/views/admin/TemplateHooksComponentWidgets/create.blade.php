...
@section("content")
    @component("admin.TemplateHooksComponentWidgets.components.card-form",['title'=>"Tema İlişki Ekle","route"=>"store"])
        ...
        <div class="form-group row mb-4">
            ...
            <div class="col-sm-12 col-md-7 h-100">
                <select name="template_hooks_id" id="template_hooks_id" class="form-control"></select>
            </div>
        </div>
                ...
                <select name="component_widget_id" id="component_widget_id" class="form-control">
                    @if(isset($component_widgets))
                        @foreach($component_widgets as $component_widget)
                            <option value="{{$component_widget->id}}">{{$component_widget->component_name}}</option>
                        @endforeach
                    @endif
                </select>
            </div>
...
@section("script")
    ...
    <script>
        $(document.body).on('change',"#template_id",function (){
            var id = $(this).val();
            $.ajax({
                type: "POST",
                url: "{{route("admin.TemplateHooksComponentWidgets.get_hook")}}",
                data:{"_token": "{{ csrf_token() }}",id:id},
                dataType: "html",
                success: function (myResponse) {
                    $("#template_hooks_id").html(myResponse);
                }
            })
        });
    </script>
