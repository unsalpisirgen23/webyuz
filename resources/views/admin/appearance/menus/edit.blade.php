                                                    ...
                                                    <li>
                                                        <label>
                                                            <input type="checkbox" name="menuItem[]"
                                                                   ...
                                                                   value='{"link":"{{$page->link}}","title":"{{$page->title}}"}'>
                                                            {{$page->title}}
                                                        </label>
                                                    </li>
                                                @endforeach
                            ...
                            <div class="card">
                                <div class="card-header p-0 ml-3" style="min-height: 40px;">
                                    ...
                                    <input type="text" name="menuTitle" class="form-control m-0 w-50 ml-3 h-100"
                                           ...
                                           style="border-radius: 0px;display: block;min-width: 310px;"  value="{{$menu->menu_title}}" >
                                    ...
                                    <div class="row">
                                        <div class="col-md-12">
                                            <div class="divRootMenuList" id="nestable3">
                                                ...
                                                <ul class="rootMenuList">{!! $content !!}</ul>
                                            </div>
                                        </div>
                                    </div>
                                </div>
    ...
    <script>
        ...
        $(document.body).on("click", "#FormPages button", function () {
            $('input[type=checkbox]:checked').each(function () {
                ...
                if (typeof item !== "undefined") {
                    ...
                    var html = `<li class="dd-item item_added page_item" data-type="page" data-title="${obj.title}" data-link="${obj.link}" >
        ...
        $(document.body).on("click", "#formCategories button", function () {
            $('input[type=checkbox]:checked').each(function () {
                ...
                if (typeof item !== "undefined") {
                    var obj = window.JSON.parse(item);
                    var html = `<li class="dd-item item_added category_item" data-type="category" data-title="${obj.title}" data-link="${obj.link}" >
                            <div class="menu-header">
                                <strong class="menuItemTitle">${obj.title}</strong>
                                <div class="buttons">
                                    <a href="" class="btn-danger delete">
        ...
        $(document).on("input", ".menuItemFromLink", function () {
            if(!$(this).hasClass("category_item") || !$(this).hasClass("category_item")){
                var link = $(this).val();
        ...
        $(document).ready(function () {
            $(document.body).on("click", "#formStore button", function () {
                if ($(".rootMenuList li").hasClass("item_added") || $(".rootMenuList li").hasClass("item_updated")) {
                    var menuTitle = $("#menuTitle").val();
                    if(menuTitle != "")
                    {
                        var content = $("#nestable3").nestable('serialize');
                        $.ajax({
                            url: "{{route("admin.appearance.menus.update")}}",
                            type: "POST",
                            data: {content:content,id:"{{$menu->id}}", menuTitle: menuTitle, "_token": "{{csrf_token()}}" },
                            success: function (res) {
                                Swal.fire({
                                    position: 'top',
                                    icon: 'success',
                                    title: 'Tebrikler menü başarıyla güncellendi.',
